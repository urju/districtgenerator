...
from . import k_medoids as k_medoids
...
def _distances(values, norm=2):
    ...
    dist = lambda day1, day2, r: math.pow(
        np.sum(np.power(np.abs(day1 - day2), r)), 1 / r
    )
...
def cluster(
    inputs, number_clusters=12, norm=2, time_limit=300, mip_gap=0.0, weights=None
):
    """
    ...
    scaling_factors = [sums_inputs[j] / sums_scaled[j] for j in range(inputs.shape[0])]
    scaled_typ_days = [
        scaling_factors[j] * typicalDays[:, j, :] for j in range(inputs.shape[0])
    ]
