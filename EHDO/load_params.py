...
from . import clustering_medoid as clustering
from . import solar_modeling as solar_modeling
...
def load_params(data):
    ...
    param = {}
    with open(
        os.path.join(os.path.dirname(srcPath), "data", "model_parameters_EHDO.json")
    ) as json_file:
    ...
    physics = {}
    with open(
        os.path.join(os.path.dirname(srcPath), "data", "physics_data.json")
    ) as json_file:
        jsonData = json.load(json_file)
        for subData in jsonData:
    ...
    start = time.time()
    (clustered_series, nc, z) = clustering.cluster(
        inputs_clustering,
        param["n_clusters"],
        norm=2,
        mip_gap=0.02,
    ...
    # load technical parameter from json-file
    # load data about central devices from JSON-file
    techParams = {}
    with open(
        os.path.join(os.path.dirname(srcPath), "data", "technical_parameters_EHDO.json")
    ) as json_file:
        jsonData = json.load(json_file)
        for subData in jsonData:
            techParams[subData["abbreviation"]] = {}
            for subsubData in subData["specifications"]:
                techParams[subData["abbreviation"]][subsubData["name"]] = subsubData[
                    "value"
                ]

    devs = copy.deepcopy(techParams)

    # Photovoltaics
    if devs["PV"]["feasible"] == "True":
        devs["PV"]["feasible"] = True
    elif devs["PV"]["feasible"] == "False":
    ...
    devs["PV"][
        "G_stc"
    ] = 1  # kW/m^2,  solar radiation under standard test conditions (STC)

    devs["PV"]["norm_power"] = (
        solar_modeling.pv_system(
            direct_tilted_irrad=param["GHI"] - param["DHI"],
    ...
    elif devs["WT"]["feasible"] == "False":
        devs["WT"]["feasible"] = False
    else:
        sys.exit("Invalid input!")

    devs["WT"]["norm_power"] = calc_WT_power(
        devs, param
    ...
    if devs["STC"]["feasible"] == "True":
        devs["STC"]["feasible"] = True
    elif devs["STC"]["feasible"] == "False":
    ...
    devs["STC"]["specific_heat"] = (
        solar_modeling.collector_system(
            direct_tilted_irrad=param["GHI"] - param["DHI"],
            diffuse_tilted_irrad=param["DHI"],
            theta=0,
            T_air=param["T_air"],
    ...
    if devs["HP"]["feasible"] == "True":
        devs["HP"]["feasible"] = True
    elif devs["HP"]["feasible"] == "False":
        devs["HP"]["feasible"] = False
    else:
        sys.exit("Invalid input!")
    devs["HP"]["COP"] = devs["HP"]["COP"] * np.ones((param["n_clusters"], 24))

    # Electric boiler
    if devs["EB"]["feasible"] == "True":
        devs["EB"]["feasible"] = True
    elif devs["EB"]["feasible"] == "False":
    ...
    # CHP
    if devs["CHP"]["feasible"] == "True":
        devs["CHP"]["feasible"] = True
    elif devs["CHP"]["feasible"] == "False":
        devs["CHP"]["feasible"] = False
    else:
        sys.exit("Invalid input!")

    # Gas boiler
    if devs["BOI"]["feasible"] == "True":
        devs["BOI"]["feasible"] = True
    elif devs["BOI"]["feasible"] == "False":
    ...
    if devs["TES"]["feasible"] == "True":
        devs["TES"]["feasible"] = True
    elif devs["TES"]["feasible"] == "False":
        devs["TES"]["feasible"] = False
    else:
        sys.exit("Invalid input!")
    deltaT = devs["TES"]["delta_T"]
    devs["TES"]["min_cap"] = (
        devs["TES"]["min_volume"] * param["rho_w"] * param["c_w"] * deltaT / 3600
    )  # kWh
    devs["TES"]["max_cap"] = (
        devs["TES"]["max_volume"] * param["rho_w"] * param["c_w"] * deltaT / 3600
    )  # kWh
    devs["TES"]["inv_var"] = devs["TES"]["inv_var"] / (
        param["rho_w"] * param["c_w"] * deltaT / 3600
    )  # EUR/kWh
    devs["TES"]["soc_init"] = 0.5  # [-], maximum initial state of charge
    ...
    # Battery
    if devs["BAT"]["feasible"] == "True":
        devs["BAT"]["feasible"] = True
    elif devs["BAT"]["feasible"] == "False":
        devs["BAT"]["feasible"] = False
    else:
        sys.exit("Invalid input!")
    devs["BAT"]["sto_loss"] = 0  # [1/h], standby losses over one time step
    devs["BAT"]["soc_init"] = (0.5,)  # [-], maximum initial state of charge
    ...
    ecoData = {}
    with open(
        os.path.join(os.path.dirname(srcPath), "data", "eco_data.json")
    ) as json_file:
        jsonData = json.load(json_file)
        for subData in jsonData:
...
def calc_reference(devs, dem, param, dem_uncl, result_dict):
            ...
            if param["ref"]["enable_chp"] and param["ref"]["enable_hp"]:
                ...
                if hp_runs:
                    ...
                    el_hp[d][t] = (
                        ...
                        (devs["CHP"]["eta_th"] / devs["CHP"]["eta_el"])
                        + param["ref"]["cop_hp"]
                    )
    ...
    # Sum up
    result_dict["ref"] = {}
    result_dict["ref"]["heat_hp_total"] = int(
        sum(
            sum(heat_hp[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["el_chp_total"] = int(
        sum(
            sum(el_chp[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["heat_boi_total"] = int(
        sum(
            sum(heat_boiler[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["cool_cc_total"] = int(
        sum(
            sum(cool_chiller[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["hydrogen_elyz_total"] = int(
        sum(
            sum(h2_elyz[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["el_dem_total"] = int(
        sum(
            sum(el_dem[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    result_dict["ref"]["gas_dem_total"] = int(
        sum(
            sum(gas_dem[d][t] for t in time_steps) * param["day_weights"][d]
            for d in days
        )
        / 1000
    )
    ...
    result_dict["ref"]["elyz_cap"] = int(
        np.max(dem_uncl["hydrogen"]) / devs["ELYZ"]["eta_el"]
    )
    ...
    result_dict["ref"]["gas_costs"] = (
        result_dict["ref"]["gas_grid_cap"] * param["price_cap_gas"]
        + result_dict["ref"]["gas_dem_total"] * 1000 * param["price_supply_gas"]
    )  # cap price and supply price

    result_dict["ref"]["el_costs"] = (
        result_dict["ref"]["el_grid_cap"] * param["price_cap_el"]
        + result_dict["ref"]["el_dem_total"] * 1000 * param["price_supply_el"]
    )  # cap price and supply price

    # CO2 tax
    result_dict["ref"]["co2_costs"] = (
        result_dict["ref"]["gas_dem_total"] * param["co2_gas"] * param["co2_tax"] * 1000
    )

    result_dict["ref"]["invest_om"] = (
        (devs["CHP"]["ann_factor"] + devs["CHP"]["cost_om"])
        * devs["CHP"]["inv_var"]
        * result_dict["ref"]["chp_cap"]
        + (devs["BOI"]["ann_factor"] + devs["BOI"]["cost_om"])
        * devs["BOI"]["inv_var"]
        * result_dict["ref"]["boiler_cap"]
        + (devs["HP"]["ann_factor"] + devs["HP"]["cost_om"])
        * devs["HP"]["inv_var"]
        * result_dict["ref"]["hp_cap"]
        + (devs["CC"]["ann_factor"] + devs["CC"]["cost_om"])
        * devs["CC"]["inv_var"]
        * result_dict["ref"]["chiller_cap"]
        + (devs["ELYZ"]["ann_factor"] + devs["ELYZ"]["cost_om"])
        * devs["ELYZ"]["inv_var"]
        * result_dict["ref"]["elyz_cap"]
    )

    result_dict["ref"]["tac"] = int(
        result_dict["ref"]["invest_om"]
        + result_dict["ref"]["gas_costs"]
        + result_dict["ref"]["el_costs"]
        + result_dict["ref"]["co2_costs"]
    ...
    result_dict["ref"]["co2"] = int(
        result_dict["ref"]["gas_dem_total"] * param["co2_gas"]
        + result_dict["ref"]["el_dem_total"] * param["co2_el_grid"]
    )  # t/a  (= t/MWh x MWh)
...
def calc_WT_power(devs, param):
    ...
    wind_speed_corr = (
        param["wind_speed"]
        * (devs["WT"]["hub_h"] / devs["WT"]["ref_h"]) ** devs["WT"]["h_coeff"]
    )  # kW
...
def get_turbine_power(wind_speed, power_curve):
    ...
    if wind_speed > power_curve[len(power_curve) - 1][0]:
        print(
            "Error: Wind speed is "
            + str(wind_speed)
            + " m/s and exceeds wind power curve table."
        )
        return 0
    ...
    for k in range(len(power_curve)):
        if power_curve[k][0] > wind_speed:
            power = (power_curve[k][1] - power_curve[k - 1][1]) / (
                power_curve[k][0] - power_curve[k - 1][0]
            ) * (wind_speed - power_curve[k - 1][0]) + power_curve[k - 1][1]
            break
    return power
